...
//===========================
//   VST PRESET — PERSONA (único padrão)
//===========================
enum VSTPreset:byte
{
  VST_PERSONA = 0,
  VST_EZDRUMMER = 1,   // mantido para compatibilidade EEPROM
  VST_SUP2TOMS = 2,    // mantido para compatibilidade EEPROM
  VST_ADDICTIVE = 3    // mantido para compatibilidade EEPROM
};
...
void loadPersonaNotes() {
  #if defined(__AVR__)
  if(EEPROM.read(799) == 0xAA) {
    // Carrega notas salvas pelo usuário
    for(byte i = 0; i < 16; i++) {
      byte v = EEPROM.read(800 + i);
      if(v <= 127) VST_NOTES_PERSONA[i] = v;
    }
  } else {
    // Primeira inicialização: usa valores já definidos em VST_NOTES_PERSONA[]
...
//===========================
void applyVSTPreset(byte vstPreset) {
  currentVSTPreset = VST_PERSONA;  // Sempre Persona
  
  // Aplica notas Persona na RAM
  for(byte i = 0; i < 16; i++) {
    Pin[i].Note = VST_NOTES_PERSONA[i];
  }
  
  // Rimshot: notas padrão Persona
  CUSTOM_NOTE_37 = 37;
  CUSTOM_NOTE_40 = 40;
  CUSTOM_RIMSHOT_FORCE_NOTE = 40;

  // ZoneDual: tabela Persona
  ...
  // Notas pads digitais: carrega da EEPROM (salvas pelo usuário)

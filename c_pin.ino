...
extern const byte   zoneDual_HoldNote[ZONE_DUAL_PAIRS];
        ...
        if(v >= VelMinimo)
        {
          ...
          for(byte p = 0; p < ZONE_DUAL_PAIRS; p++) {
            if(i == zoneDual_PadIdx[p] && zoneDual_Active[p]) {
              ...
              setMonitorSensor(i);
              // Se hold-choke já disparou para este par, não envia a nota de borda
              if(zoneDual_HoldNote[p] && zoneDual_HoldFired[p]) {
                zoneDual_EdgeFired[p] = false; // choke ganhou, ignora borda

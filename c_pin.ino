...
inline void setMonitorSensor(byte s) {
  if(monitorVelocityAtivo) monitorLastSensor = s;
}
...
//===========================
//   VST PRESETS - AGORA COM 4 OPÇÕES
//===========================
enum VSTPreset:byte
{
  VST_PERSONA = 0,         // NOVO: Persona (notas salvam)
  VST_EZD_SUPERIOR = 1,    // SupEzd1
  VST_SUP2TOMS = 2,        // SupEzd2
  VST_ADDICTIVE = 3        // Addictive
};

byte currentVSTPreset = VST_PERSONA; // Padrão: Persona

//===========================
//   TABELAS DE NOTAS POR VST
//===========================

// *** NOVO: TABELA PERSONA (inicializa com padrão SupEzd1, mas salva alterações) ***
byte VST_NOTES_PERSONA[16] = {
...
// TABELA PARA EZDRUMMER 3 / SUPERIOR 3 (SupEzd1)
const byte VST_NOTES_EZD[16] PROGMEM = {
  4,   // Pad 0  - C CHIMBAL (HHC)
  8,   // Pad 1  - CHIMBAL   
  38,  // Pad 2  - CAIXA  
  37,  // Pad 3  - ARO    
...
// TABELA PARA SUPERIOR 2 TOMS (SupEzd2)
const byte VST_NOTES_SUP2TOMS[16] PROGMEM = {
  4,   // Pad 0  - C CHIMBAL (HHC)
  8,   // Pad 1  - CHIMBAL   
  38,  // Pad 2  - CAIXA  
  37,  // Pad 3  - ARO    
...
//===========================
//   FUNÇÃO PARA OBTER NOTA BASEADA NO VST
//===========================
byte getVSTNote(byte pad) {
  if(pad >= 16) return 0;
  
  if(currentVSTPreset == VST_PERSONA) {
    // *** PERSONA: retorna da tabela RAM (modificável) ***
    return VST_NOTES_PERSONA[pad];
  }
  else if(currentVSTPreset == VST_ADDICTIVE) {
    return pgm_read_byte(&VST_NOTES_ADDICTIVE[pad]);
  } 
  else if(currentVSTPreset == VST_SUP2TOMS) {
    return pgm_read_byte(&VST_NOTES_SUP2TOMS[pad]);
  } 
  else {
    return pgm_read_byte(&VST_NOTES_EZD[pad]);
  }
}

//===========================
//   *** NOVO: SALVA NOTA PERSONA NA EEPROM ***
//===========================
void savePersonaNote(byte pad, byte note) {
  if(pad >= 16) return;
  #if defined(__AVR__)
  // Endereço base: 800 + pad
  EEPROM.write(800 + pad, note);
  #endif
}

//===========================
//   *** NOVO: CARREGA NOTAS PERSONA DA EEPROM ***
//===========================
void loadPersonaNotes() {
  #if defined(__AVR__)
  // Verifica se há dados salvos (marca de validação)
  byte validMark = EEPROM.read(799);
  
  if(validMark == 0xAA) {
    // Carrega notas salvas
    for(byte i = 0; i < 16; i++) {
      VST_NOTES_PERSONA[i] = EEPROM.read(800 + i);
      // Validação de segurança
      if(VST_NOTES_PERSONA[i] > 127) {
        VST_NOTES_PERSONA[i] = pgm_read_byte(&VST_NOTES_EZD[i]);
      }
    }
  } else {
    // Primeira vez: inicializa com padrão SupEzd1
    for(byte i = 0; i < 16; i++) {
      VST_NOTES_PERSONA[i] = pgm_read_byte(&VST_NOTES_EZD[i]);
    }
    // Marca como inicializado
    EEPROM.write(799, 0xAA);
    for(byte i = 0; i < 16; i++) {
      savePersonaNote(i, VST_NOTES_PERSONA[i]);
    }
  }
  #endif
}
...
//===========================
void applyVSTPreset(byte vstPreset);
...
//===========================
//   FUNÇÃO PARA APLICAR PRESET VST - MODIFICADA PARA PERSONA
//===========================
void applyVSTPreset(byte vstPreset) {
  currentVSTPreset = vstPreset;
  
  // Aplica notas na RAM apenas - sem gravar na EEPROM
  // (presets EZD/SUP2TOMS/ADDICTIVE sao fixos em PROGMEM, nao precisam ser salvos)
  // (Persona ja salva suas notas individualmente via savePersonaNote)
  for(byte i = 0; i < 16; i++) {
    Pin[i].Note = getVSTNote(i);
  }
  
  // Configura notas de rimshot baseadas no VST
  if(vstPreset == VST_ADDICTIVE) {
    CUSTOM_NOTE_37 = 42;
    CUSTOM_NOTE_40 = 37;
    CUSTOM_RIMSHOT_FORCE_NOTE = 37;

    // *** ADDICTIVE: tabela ZoneDual com notas do Addictive Drummer 2 ***
    // Par | Aux nota | Bloqueia | Piezo
    //  1  |    61    |    60    |  A8  (Ride cond→cup)
    //  2  |    62    |    60    |  A8  (Ride cond→campana)
    //  3  |     7    |     8    |  A1  (Chimbal zona extra)
    //  4  |     9    |     8    |  A1  (Chimbal cúpula)
    //  5  |    81    |    77    |  A10 (Crash 1 zona)
    //  6  |    89    |    79    |  A11 (Crash 2 zona)
    ...
    // *** ADDICTIVE: atualiza notas do menu Pad Digital (fixas, nao salva) ***
    ...
    // *** PERSONA, SupEzd1, SupEzd2: rimshot e ZoneDual iguais ***
    CUSTOM_NOTE_37 = 37;
    CUSTOM_NOTE_40 = 40;
    CUSTOM_RIMSHOT_FORCE_NOTE = 40;

    // *** PERSONA / EZD / SUP2TOMS: tabela ZoneDual original ***
    zoneDual_NoteAux[0]   = 53; zoneDual_NoteBlock[0] = 51; zoneDual_PadIdx[0] =  8;
    zoneDual_NoteAux[1]   = 59; zoneDual_NoteBlock[1] = 51; zoneDual_PadIdx[1] =  8;
    zoneDual_NoteAux[2]   =  7; zoneDual_NoteBlock[2] =  8; zoneDual_PadIdx[2] =  1;
    zoneDual_NoteAux[3]   =  9; zoneDual_NoteBlock[3] =  8; zoneDual_PadIdx[3] =  1;
    zoneDual_NoteAux[4]   = 27; zoneDual_NoteBlock[4] = 49; zoneDual_PadIdx[4] = 10;
    zoneDual_NoteAux[5]   = 31; zoneDual_NoteBlock[5] = 57; zoneDual_PadIdx[5] = 11;

    if(vstPreset == VST_PERSONA) {
      // *** PERSONA: carrega notas salvas pelo usuario na EEPROM ***
      LoadDigitalNotesEEPROM();
    } else {
      // *** SupEzd1 / SupEzd2: notas FIXAS, nao salva na EEPROM ***
      digitalPadNotes[0] = 53;
      digitalPadNotes[1] = 59;
      digitalPadNotes[2] =  7;
      digitalPadNotes[3] =  9;
  ...
  // Sem markPinChanged, sem markAdvancedChanged, sem markVSTChanged
  // Troca de preset nao gasta EEPROM
}
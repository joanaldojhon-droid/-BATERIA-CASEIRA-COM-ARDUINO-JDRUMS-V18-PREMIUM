...
struct PadPreset {
  ...
  byte antiCrosstalk;  // Anti-Crosstalk local por pad (0-100) — filtro de ghost note por força relativa
};
 ...
 // A0 - CONTROLE CHIMBAL (HHC)
  {
   HHC,      // type: Hi-Hat Controller
    ...
    0         // AntiCros: HHC nao precisa filtro
  },
    // A1 - CHIMBAL 
  {
    Piezo,    // type: Piezo
...
class pin
{
  ...
  {    
    ...
    AntiCrosstalk=0;  // *** Anti-Crosstalk local por pad (0-100) — padrão 0 (desligado) ***
    ...
    {          
      if(Type==Piezo)
      {
        ...
        if(v >= VelMinimo)
        {
          ...
          if(!zoneDualHandled)
          {
          // *** CAMADA 2: XCancel Global — filtro automático de crosstalk entre pads ***
          ...
          if(!shouldBlock) {
            // *** CAMADA 3: AntiCrosstalk local — filtro ghost note por força relativa ***
            // Compara a batida atual com a última batida forte DO MESMO PAD.
            // Bloqueia se a batida for muito fraca em relação à anterior (vibração mecânica).
            // Em rolos/rufes cada batida vira a nova "forte" → não bloqueia sequência rápida.
            bool acBlock = false;

            if(AntiCrosstalk > 0) {
              unsigned int janela    = 80 + AntiCrosstalk;               // 80-180ms
              float threshold        = 0.50 + (AntiCrosstalk * 0.003f);  // 50%-80%
              byte  velocityMin      = 10 + (byte)(AntiCrosstalk * 0.15f); // 10-25
              unsigned int janelaVel = 120 + AntiCrosstalk;              // 120-220ms
              byte  batidaForte      = 40 - (byte)(AntiCrosstalk * 0.15f); // 40-25
              ...
              // FILTRO 2: Velocity mínima dinâmica
            ...
            if(!acBlock) {
              ...
              // Registra este pad como último hit forte (XCancel Global)
              gLastStrongHit  = MaxReading;
              gLastStrongTime = currentTime;
              gLastStrongPad  = i;
              // Registra batida forte local (AntiCrosstalk)
              if(AntiCrosstalk > 0) {
                byte batidaForte = 40 - (byte)(AntiCrosstalk * 0.15f);
                if(v > batidaForte) {
  ...
  byte AntiCrosstalk; // *** Anti-Crosstalk local por pad (0-100): filtro ghost note por força relativa ***

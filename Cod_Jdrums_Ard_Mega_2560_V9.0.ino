...
//=>  • Silvio e Joanaldo - Testes de hardware e MIDI                                      <=
//=>  HARDWARE:                                                                            <=
//=>  • Arduino Mega 2560                                                                  <=
...
//=>  • Sistema dual e tricone Ride Chimbal Crash 1 e Crash 2                              <=
...
byte CUSTOM_NOTE_38 = 38;           // Nota CAIXA — sincroniza com Pin[2].Note (padrão 38)
...
const char name_18[] PROGMEM = "CRASH 1    ";
const char name_19[] PROGMEM = "CRASH 2    ";
const char name_20[] PROGMEM = "CRASH 3    ";
const char name_21[] PROGMEM = "CAIXA REVEB";
const char name_22[] PROGMEM = "ROTOMTOM   ";
const char name_23[] PROGMEM = "GOMBELL    ";
const char name_24[] PROGMEM = "SURDO 1    ";
const char name_25[] PROGMEM = "SURDO 2    ";
const char name_26[] PROGMEM = "PAD EFET 1 ";
const char name_27[] PROGMEM = "PAD EFET 2 ";
const char name_28[] PROGMEM = "PAD EFET 3 ";
const char name_29[] PROGMEM = "PAD EFET 4 ";
const char name_30[] PROGMEM = "CON CHIMBAL";
const char name_31[] PROGMEM = "CHIMBAL    ";
const char name_32[] PROGMEM = "CAIXA      ";
const char name_33[] PROGMEM = "ARO CAIXA  ";
const char name_34[] PROGMEM = "BUMBO      ";
  ...
  if (!drumBuffer.window_open) {
    ...
    if (drumBuffer.note38_active && (currentTime - drumBuffer.note38_time) > MAX_NOTE_AGE_MS) {
      ...
      if (drumBuffer.note38_velocity >= VELOCITY_MIN_NOTE38) {
        sendMidiNote(drumBuffer.note38_channel, CUSTOM_NOTE_38, drumBuffer.note38_velocity);
      }
...
// CORREÇÃO v5.0: O buffer rimshot (37/38->40) SO intercepta os pads A2 (caixa) e A3 (aro).
// Antes, qualquer pad cuja nota coincidisse com CUSTOM_NOTE_37 (42) ou CUSTOM_NOTE_38 (38)
// era capturado pelo buffer — fazendo pads A4-A15 travarem ou dispararem nota errada
// quando o usuario mudava a nota para 38 ou 42 pelo menu.
// A variavel monitorLastSensor (setada por setMonitorSensor(i) antes de cada fastNoteOn)
// identifica qual pad esta disparando. So pads 2 e 3 entram no buffer rimshot.
// Pads 0,1 e 4-15 com nota coincidente passam direto para sendMidiNote.
//
// Indices: A0=0 (HHC), A1=1 (Chimbal), A2=2 (Caixa), A3=3 (Aro), A4-A15=4-15
#if ENABLE_CHANNEL
  #define fastNoteOn(_channel,_note,_velocity) { \
    bool _isRimshotPad = (monitorLastSensor == 2 || monitorLastSensor == 3); \
    if (ENABLE_RIMSHOT_38_TO_40 && _note == CUSTOM_NOTE_38 && _velocity > 125 && _isRimshotPad) { \
      sendMidiNote(_channel, CUSTOM_RIMSHOT_FORCE_NOTE, _velocity); \
    } \
    else if (ENABLE_NOTE_37_38_TO_40 && (_note == CUSTOM_NOTE_37 || _note == CUSTOM_NOTE_38) && _isRimshotPad) { \

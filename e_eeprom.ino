...
void LoadAllEEPROM()
{
  ...
  if(Version==41)  // *** v41: inclui XCanCost (param 0x0F) no load/save ***
  {
    ...
    // *** XCanCost por pad — endereço próprio (686-701) ***
    LoadXCanCostEEPROM();
    ...
    // *** Aplica modo Comp/Forc salvo (rimSub) aos enables ***
    if(rimSub == 0) {
  ...
  else
  {
    ...
    // *** Grava XCanCost por pad (endereços 686-702) ***
    SaveXCanCostEEPROM();
...
void LoadEEPROM(byte Pin, byte Param)
{
  ...
  // *** AntiCrosstalk oculto: sempre 0, ignora EEPROM velha ***
  if(Param == ANTICROSSTALK) { ExecCommand(0x03,Pin,Param,0); return; }

  if(Param == RETRIGGER) {
    // 255 = Aut — valor válido para RETRIGGER, aplica normalmente
  } else {
    // *** EEPROM virgem (255): ignora e mantém o valor do PAD_PRESETS ***
    // Só aplica se o usuário já salvou um valor real (0-127)
    if(Value == 255) return;
...
void LoadAdvancedEEPROM(byte Param)
{
  ...
  switch(Param)
  {
    ...
    case 11:  // *** MODIFICAÇÃO 2: carrega rimSub (Comp/Forc do A2) ***
      rimSub = (Value <= 1) ? Value : 0;
...
void SaveAllToEEPROMImmediate()
{
  ...
  SaveXCanCostEEPROM();     // *** XCANCOST POR PAD ***
...
// *** XCANCOST POR PAD — endereços 686 a 701 (16 bytes, um por porta A0-A15) ***
#define EEPROM_XCANCOST_BASE  686
#define EEPROM_XCANCOST_FLAG  702  // flag de validade: 0xDD = dados gravados

void SaveXCanCostEEPROM() {
  #if defined(__AVR__)
  for(byte i = 0; i < 16; i++) {
    byte v = Pin[i].XCanCost;
    if(v > 127) v = 127;
    EEPROM.write(EEPROM_XCANCOST_BASE + i, v);
  }
  EEPROM.write(EEPROM_XCANCOST_FLAG, 0xDD);
  #endif
}

void LoadXCanCostEEPROM() {
  #if defined(__AVR__)
  if(EEPROM.read(EEPROM_XCANCOST_FLAG) != 0xDD) return; // EEPROM virgem — mantém PAD_PRESETS
  for(byte i = 0; i < 16; i++) {
    byte v = EEPROM.read(EEPROM_XCANCOST_BASE + i);
    if(v <= 127) Pin[i].XCanCost = v;
  }
  #endif
}

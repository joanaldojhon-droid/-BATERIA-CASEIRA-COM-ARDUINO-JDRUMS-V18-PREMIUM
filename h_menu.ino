...
// =============================================================
// MENSAGEM PADRAO MIDI — CONTROLADO POR HOLD 10s DOS BOTOES
// Botao A hold 10s = toggle mensagem ao LIGAR  (e controla se VST e salvo na EEPROM)
// Botao B hold 10s = toggle mensagem ao TROCAR no menu
// Salvo na EEPROM enderecos 579 e 580
//
// vstMsgBootAtivo = true  → grava VST na EEPROM, volta no ultimo padrao ao ligar
// vstMsgBootAtivo = false → NAO grava VST na EEPROM, sempre volta em Person ao ligar
// =============================================================
bool vstMsgBootAtivo = true;   // mensagem ao LIGAR (padrao: ativo)
bool vstMsgMenuAtivo = true;   // mensagem ao TROCAR no menu (padrao: ativo)
#define EEPROM_VST_MSG_BOOT  579
#define EEPROM_VST_MSG_MENU  580
#define HOLD_VST_MSG_MS  10000UL  // 10 segundos
...
byte digitalPadNotes[7] = {53, 59,  7,  9, 27, 31, 90};  // Valores padrão Aux1-7 (ZoneDual)
...
byte chokeNotes[4] = {78, 80, 82, 63};  // Crash1, Crash2, Crash3, Ride
byte eMenuChokeNote = 0;             // Índice do choke (0-3)

// *** FILTRO HOLD CHOKE (transistor anti-ruído) - EEPROM 585-589 ***
// chokeHoldAtivo: 0=desativado (chapinha normal), 1=ativado (transistor com HOLD)
// chokeHoldMs[4]: tempo HOLD individual por choke (0-80ms) — idx 0=Crash1/P53, 1=Crash2/P51, 2=Crash3/P49, 3=Ride/P47
#define EEPROM_CHOKE_HOLD_ATIVO  585          // 1 byte: flag ativo/desativado
#define EEPROM_CHOKE_HOLD_BASE   586          // 4 bytes: hold individual (586-589)
byte chokeHoldAtivo    = 0;                   // 0=OFF, 1=ON
byte chokeHoldMs[4]    = {3, 3, 3, 3};      // valores padrão: Crash1, Crash2, Crash3, Ride
...
// *** HOLD 10s — toggle mensagens VST ***
unsigned long vstMsgHoldA_Start = 0;
unsigned long vstMsgHoldB_Start = 0;
bool vstMsgHoldA_Active = false;
bool vstMsgHoldB_Active = false;
...
#define S_DIGITAL_MENU PSTR("Pad Digital NT")
...
#define S_CHOKE_FILTER   PSTR("Filtro")
#define S_CHOKE_FILT_OFF PSTR("Desativ")
#define S_CHOKE_FIT_CR1  PSTR("FitCra1 P53")
#define S_CHOKE_FIT_CR2  PSTR("FitCra2 P51")
#define S_CHOKE_FIT_CR3  PSTR("FitCra3 P49")
#define S_CHOKE_FIT_RIDE PSTR("FitRide P47")
...
#define S_ANTICROSSTALK PSTR("AntiCros")  
...
// *** NOVO: VST COM PERSONA EM PRIMEIRO ***
#define S_PERSONA PSTR("Person")
#define S_EZDRUMMER PSTR("SupEzd1")
#define S_SUP2TOMS PSTR("SupEzd2")  
#define S_ADDICTIVE PSTR("Addctiv")

#define VST_PERSONA 0
#define VST_EZDRUMMER 1
#define VST_SUP2TOMS 2
#define VST_ADDICTIVE 3

// *** MENSAGEM DE TROCA DE PADRÃO MIDI (2 segundos) ***
void showVSTChangeMessage(byte preset) {
  lcd.clear();
  // Linha 1: nome do padrão
  lcd.setCursor(0, 0);
  switch(preset) {
    case VST_PERSONA:   lcd.print("Padrao Person"); break;
    case VST_EZDRUMMER: lcd.print("Padrao SupEze1"); break;
    case VST_SUP2TOMS:  lcd.print("Padrao SupEze2"); break;
    case VST_ADDICTIVE: lcd.print("Padrao Addctiv"); break;
  }
  // Linha 2: mensagem específica
  lcd.setCursor(0, 1);
  if(preset == VST_PERSONA) {
    lcd.print("Salva As Notas");
  } else {
    lcd.print("Nao Salva Notas");
  }
  delay(2000);
  needsRedraw = true;
}
...
void encoderEnterLevel() {
  ...
  if(eMenuPage == 25 && eMenuSelect == 1) {
    switch(eMenuConfig) {
      ...
      case 1:  eMenuPage = 19; break;
      case 2:  eMenuPage = 20; break;
      case 3:  eMenuPage = 21; break;
      case 4:  eMenuPage = 22; break;
...
void CheckNamesIntegrity()
{
  ...
  for(byte i = 0; i < 16; i++) {
    if(selected_names[i] >= NUM_AVAILABLE_NAMES || selected_names[i] > 24 || selected_names[i] == 255) {
...
void PrintCustomName(byte pin, bool sel)
{
  if(pin < 16) {
    ...
    if(selected_names[pin] >= NUM_AVAILABLE_NAMES || selected_names[pin] > 24 || selected_names[pin] == 255) {
...
void printNameFromIndex(byte nameIdx, bool sel) {
  if(nameIdx >= NUM_AVAILABLE_NAMES || nameIdx > 24) nameIdx = 0;
...
void GetProMicroName(byte padIndex, char* buffer) {
  ...
  if(nameIdx >= NUM_AVAILABLE_NAMES || nameIdx > 24) {
...
void startNameEdit(byte pin) {
  ...
  editingName[12] = '\0';
  resetNameEditButtons = true;
  needsRedraw = true;
}

void exitNameEdit(bool save) {
  ...
  if(save) {
    ...
    #if defined(__AVR__)
    int address = 600 + (editingPin * 13);
    for(int i = 0; i < 12; i++) {
      char charToSave = (editingName[i] != '\0') ? editingName[i] : ' ';
      EEPROM.write(address + i, charToSave);
    }
    EEPROM.write(address + 12, '\0');
    EEPROM.write(600 + 208 + editingPin, 1);
    delay(100);
    #endif
    markNameChanged();
    lcd.clear();
  ...
  btnB_Time = 0;
  eMenuSelect = 1;
  eMenuPin = 14;  // *** NAME agora é eMenuPin 14 ***
  needsRedraw = true;
  lcd.clear();
  delay(50);
  Draw();
...
//==============================
//    FUNÇÕES MENSAGENS VST - SAVE/LOAD EEPROM
//    579 = vstMsgBootAtivo  580 = vstMsgMenuAtivo
//==============================
void SaveVstMsgEEPROM() {
  #if defined(__AVR__)
  EEPROM.write(EEPROM_VST_MSG_BOOT, vstMsgBootAtivo ? 1 : 0);
  EEPROM.write(EEPROM_VST_MSG_MENU, vstMsgMenuAtivo ? 1 : 0);
  #endif
}

void LoadVstMsgEEPROM() {
  #if defined(__AVR__)
  byte vBoot = EEPROM.read(EEPROM_VST_MSG_BOOT);
  byte vMenu = EEPROM.read(EEPROM_VST_MSG_MENU);
  vstMsgBootAtivo = (vBoot == 0) ? false : true;
  vstMsgMenuAtivo = (vMenu == 0) ? false : true;
  #endif
}
...
void resetToFactoryDefaults() {
  ...
  // --- Restaura notas Persona para valores de fábrica (VST_NOTES_EZD do PROGMEM) ---
  // Sem isso, applyVSTPreset leria as notas modificadas pelo usuário ainda em RAM/EEPROM
  #if defined(__AVR__)
  for(byte i = 0; i < 16; i++) {
    VST_NOTES_PERSONA[i] = pgm_read_byte(&VST_NOTES_EZD[i]);
    EEPROM.write(800 + i, VST_NOTES_PERSONA[i]);  // Grava direto — reset é síncrono
  }
  // Marca de validação 0xAA no endereço 799 permanece intacta
  // loadPersonaNotes() no próximo boot vai carregar os valores restaurados
  #endif

  // --- Aplica preset Persona com os valores de fábrica restaurados ---
  currentVSTPreset = VST_PERSONA;
  applyVSTPreset(VST_PERSONA);
  ...
  // --- Notas pads digitais Aux 1-7 ---
  byte defDigital[7] = {53, 59,  7,  9, 27, 31, 90};
  ...
  // --- Invert Sensor por pad: tudo desativado ---
...
void LoadDigitalNotesEEPROM() {
  ...
  byte defaults[7] = {53, 59,  7,  9, 27, 31, 90};
...
void SaveChokeFilterEEPROM() {
  #if defined(__AVR__)
  EEPROM.write(EEPROM_CHOKE_HOLD_ATIVO, chokeHoldAtivo);
  for(byte i = 0; i < 4; i++) {
    EEPROM.write(EEPROM_CHOKE_HOLD_BASE + i, chokeHoldMs[i]);
  }
  #endif
}

void LoadChokeFilterEEPROM() {
  #if defined(__AVR__)
  byte defaults[4] = {3, 3, 3, 3};
  byte fa = EEPROM.read(EEPROM_CHOKE_HOLD_ATIVO);
  chokeHoldAtivo = (fa <= 1) ? fa : 0;
  for(byte i = 0; i < 4; i++) {
    byte v = EEPROM.read(EEPROM_CHOKE_HOLD_BASE + i);
    chokeHoldMs[i] = (v <= 80) ? v : defaults[i];
  }
  #endif
}
...
void startConfirmation(byte pin) {
  if(pin >= 16) return;
  confirmationMode = true;
  confirmingPin = pin;
  confirmationStartTime = millis();
  resetConfirmButtons = true;
  needsRedraw = true;
}
...
void exitConfirmation(bool confirmed) {
  ...
  if(confirmed) {
    clearCustomName(confirmingPin);
    selected_names[confirmingPin] = 0;
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print("LIMPANDO");
    delay(1000);
...
void processEncoder() {
  ...
  if(currentEncoderButtonState) {
    ...
    // Hold longo (3s): ativa atalho PAD (3 bips)
    if(held >= RESETDELAY && !padShortcutActivated) {
      if(!nameEditMode
         && !confirmationMode
         && !proMicroNameEditMode
    ...
    else if(held >= HOLDDELAY && !holdMediumFired && !padShortcutActivated) {
...
void NameEditMenu() {
  ...
  // *** IGUAL AO PADS AUXILIAR: espera soltar A+B antes de aceitar qualquer comando ***
  static bool botoesForamSoltos = false;
  if(resetNameEditButtons) { botoesForamSoltos = false; resetNameEditButtons = false; }
  if(!botoesForamSoltos) {
    if(btnA == HIGH && btnB == HIGH) {
      botoesForamSoltos = true;
      btnA_State = 0; btnB_State = 0;
...
void ConfirmationMenu() {
  ...
  static bool botoesForamSoltos = false;
  static bool telaJaDesenhada = false;
  if(resetConfirmButtons) { botoesForamSoltos = false; telaJaDesenhada = false; resetConfirmButtons = false; }
  ...
  if(currentEncoderButtonState && !lastEncoderButtonState) {
    ...
    unsigned long pressDuration = millis() - encoderButtonPressTime;
    telaJaDesenhada = false; botoesForamSoltos = false;
    if(pressDuration >= 1000) exitConfirmation(true);
    else exitConfirmation(false);
    lastEncoderButtonState = currentEncoderButtonState;
    return;
  }
  lastEncoderButtonState = currentEncoderButtonState;
  ...
  if(btnLimpaPressionado && !btn_limpa_pressed) {
    btn_limpa_pressed = true;
    delay(50); telaJaDesenhada = false; botoesForamSoltos = false;
    exitConfirmation(true); return;
  } else if(!btnLimpaPressionado && btn_limpa_pressed) { btn_limpa_pressed = false; }

  if(btnCancelaPressionado && !btn_cancela_pressed) {
    btn_cancela_pressed = true;
    delay(50); telaJaDesenhada = false; botoesForamSoltos = false;
    exitConfirmation(false); return;
  } else if(!btnCancelaPressionado && btn_cancela_pressed) { btn_cancela_pressed = false; }
...
void Menu()
{
  ...
  // *** HOLD 10s BOTAO A — toggle mensagem de boot + controle de gravação VST ***
  // Quando ativado:  grava VST na EEPROM, volta no ultimo padrao ao ligar
  // Quando desativado: NAO grava VST, sempre volta em Person ao ligar
  if(btnA == LOW && btnB == HIGH) {
    if(!vstMsgHoldA_Active) {
      vstMsgHoldA_Active = true;
      vstMsgHoldA_Start = now;
    } else if(now - vstMsgHoldA_Start >= HOLD_VST_MSG_MS) {
      vstMsgHoldA_Active = false;
      vstMsgBootAtivo = !vstMsgBootAtivo;
      SaveVstMsgEEPROM();
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Ms Inicializacao");
      lcd.setCursor(0, 1);
      if(vstMsgBootAtivo) lcd.print("   ATIVADA      ");
  ...
  // *** HOLD 10s BOTAO B — toggle mensagem ao trocar padrao no menu ***
  if(btnB == LOW && btnA == HIGH) {
    if(!vstMsgHoldB_Active) {
      vstMsgHoldB_Active = true;
      vstMsgHoldB_Start = now;
    } else if(now - vstMsgHoldB_Start >= HOLD_VST_MSG_MS) {
      vstMsgHoldB_Active = false;
      vstMsgMenuAtivo = !vstMsgMenuAtivo;
      SaveVstMsgEEPROM();
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Msg Padrao Midi ");
      lcd.setCursor(0, 1);
      if(vstMsgMenuAtivo) lcd.print("   ATIVADA      ");
      else                lcd.print("  DESATIVADA    ");
      #if BUZZER
      if(buzzerEnabled) { playBeep(); delay(80); playBeep(); }
      #endif
  ...
  if (btnB == LOW && btnA == LOW) {
    ...
    // *** PADS PRINCIPAL (páginas 2-17, parâmetro NAME=14): A+B 1.5s → edita nome ***
    else if(eMenuPage >= 2 && eMenuPage < 18 && eMenuPin == 14) {
      if(!bothButtonsPadsAuxPressed) {
        bothButtonsPadsAuxPressed = true;
        bothButtonsPadsAuxTime = now;
      }
      else if(now - bothButtonsPadsAuxTime >= 1500) {
        bothButtonsPadsAuxPressed = false;
        ...
        #endif
        #if defined(__AVR__)
        bool hasCustom = (EEPROM.read(600 + 208 + pinIndex) == 1);
        #else
        bool hasCustom = false;
        #endif
        if(hasCustom) {
          startConfirmation(pinIndex);
        } else {
          // Garante que está em nível 1 antes de entrar em edição
          eMenuSelect = 1;
          startNameEdit(pinIndex);
        }
      }
      return;  // Nunca reseta neste contexto
    }
    // *** PADS AUXILIAR (página 24, parâmetro NAME=11, nível 2): A+B 1.5s → edita nome ***
      ...
      if(!bothButtonsPadsAuxPressed) {
        ...
        if(hasCustom) {
          ...
          startProMicroNameEdit(eMenuProMicro);
        }
      }
      return;
    }
    // *** BACKUP PRESETS (página 20, itens 0/1/2): A+B 1.5s → edita nome do preset ***
    else if(eMenuPage == 20 && eMenuSelect == 1 && eMenuBackup >= 0 && eMenuBackup <= 2) {
      if(!bothButtonsPadsAuxPressed) {
        bothButtonsPadsAuxPressed = true;
        bothButtonsPadsAuxTime = now;
      }
        ...
        bothButtonsPadsAuxPressed = false;
        byte pNum = eMenuBackup + 1;
        #if BUZZER
        if(buzzerEnabled) playBeep();
        #endif
        if(hasPresetCustomName(pNum)) {
          presetConfirmMode     = true;
          confirmingPresetNum   = pNum;
          confirmationStartTime = millis();
          resetPresetConfirmButtons = true;
          needsRedraw = true;
        } else {
          startPresetNameEdit(pNum);
        }
      }
      return;  // Nunca reseta neste contexto
    }
    // *** PÁGINA 20 (outros itens): não reseta ***
...
void Up()
{
    ...
    // *** DENTRO DE SUBMENU: loop entre submenus ***
      ...
      if(eMenuPage == 25) {
        ...
        switch(eMenuConfig) {
          ...
          case 1: eMenuPage = 19; eMenuSelect = 0; break;
          case 2: eMenuPage = 20; eMenuSelect = 0; break;
          case 3: eMenuPage = 21; eMenuSelect = 0; break;
          case 4: eMenuPage = 22; eMenuSelect = 0; break;
      ...
      // Se saiu do pad A0 (pág 2) com InvSensor selecionado → reseta para Note
      if(eMenuPage == 3 && eMenuPin == 15) eMenuPin = 0;
  ...
  else if(eMenuSelect==1)
  {
    if(eMenuPage==0) Mode=(mode)(((int)Mode+1)%4);
      ...
      // ✅ Pula índices 4, 6, 8 (ocultos) — novos itens 9,10,11,12 visíveis
      eMenuAdvanced = (eMenuAdvanced + 1) % 13;
      if(eMenuAdvanced == 4) eMenuAdvanced = 5;   // Pula 4
      else if(eMenuAdvanced == 6) eMenuAdvanced = 7;   // Pula 6
      else if(eMenuAdvanced == 8) eMenuAdvanced = 9;   // Pula 8, vai para 9
      else if(eMenuAdvanced == 12) eMenuAdvanced = 0;  // Pula 12 (oculto)
    }
    else if(eMenuPage==20) eMenuBackup=(eMenuBackup+1)%6;
    else if(eMenuPage==21) eMenuBuzzer=(eMenuBuzzer+1)%1;
    else if(eMenuPage==22) eMenuVST=(eMenuVST+1)%1;
    else if(eMenuPage==23) eMenuMidi=(eMenuMidi+1)%2;
    else if(eMenuPage==24) eMenuProMicroParam=(eMenuProMicroParam+1)%12;
      ...
      // 0-3=notas, 4=Filtro, 5-8=FitCra1/FitCra2/FitCra3/FitRide (só quando ativo)
      byte maxItem = chokeHoldAtivo ? 8 : 4;
      eMenuChokeNote = (eMenuChokeNote + 1) % (maxItem + 1);
    }  // *** NOVO: Página 28 - Notas Choke (0-3=notas, 4=filtro, 5-8=hold individual) ***
    // *** MENU XCANCEL (página 34) — navega entre os 16 pares ***
      ...
      // ✅ Volta ao início após o 13
      if(eMenuConfig > 13) {
        eMenuConfig = 0;
      }
    }
    else if(eMenuPage>=2 && eMenuPage<18) {
      eMenuPin=(eMenuPin+1)%16;
      // InvSensor (15) só aparece no pad A0 (eMenuPage==2)
  ...
  else if(eMenuSelect==2)
  {
    ...
    else if(eMenuPage==19)
    {
      ...
      else if(eMenuAdvanced==5) { CUSTOM_NOTE_40=(CUSTOM_NOTE_40+1)%128; markAdvancedChanged(); }
    ...
    else if(eMenuPage==22)
    {
      if(eMenuVST==0) { 
        currentVSTPreset=(currentVSTPreset+1)%4;
        applyVSTPreset(currentVSTPreset);
        if(vstMsgBootAtivo) markVSTChanged();  // so grava na EEPROM se boot msg ativo
        if(vstMsgMenuAtivo) showVSTChangeMessage(currentVSTPreset);
      }
    }
    ...
    else if(eMenuPage==27)
    {
      ...
      // Só salva na EEPROM se estiver no padrão Person (os outros têm notas fixas)
      if(currentVSTPreset == VST_PERSONA) SaveDigitalNotesEEPROM();
    }
    // *** NOVO: Notas Choke (28) ***
    else if(eMenuPage==28)
    {
      ...
      } else if(eMenuChokeNote == 4) {
        // Item Filtro: alterna ON/OFF
        chokeHoldAtivo = chokeHoldAtivo ? 0 : 1;
        // Se desativou, volta seleção para 4 (itens 5-8 somem)
        if(!chokeHoldAtivo && eMenuChokeNote > 4) eMenuChokeNote = 4;
        SaveChokeFilterEEPROM();
      } else {
        // Itens 5-8: ajuste hold individual (loop 0-80)
        byte idx = eMenuChokeNote - 5;  // 5→0, 6→1, 7→2, 8→3
        chokeHoldMs[idx] = (chokeHoldMs[idx] + 1) % 81;
        SaveChokeFilterEEPROM();
      }
    }
    ...
    else if(eMenuPage==24)
    {
      ...
      switch(eMenuProMicroParam) {
        ...
        case 3: // MaskTime
          ...
          break;
        case 4: // Retrigger
          currentValue = proMicroPads[eMenuProMicro].retrigger;
          paramCode = PARAM_RETRIGGER;
          break;
        case 5: // Curve (0-4)
        ...
        case 9: // Channel
          ...
          break;
        case 10: // AntiCrosstalk
          currentValue = proMicroPads[eMenuProMicro].antiCrosstalk;
          paramCode = PARAM_ANTICROSSTALK;
          break;
        case 11: // Name
          #if defined(__AVR__)
          bool hasCustom = (EEPROM.read(875 + eMenuProMicro) == 1);
    ...
    else if(eMenuPage>=2 && eMenuPage<18)
    {
      ...
      switch(eMenuPin)
      {
        case 0: // Note
          ADD(Note);
          if(currentVSTPreset == VST_PERSONA && pinIndex < 16) {
            VST_NOTES_PERSONA[pinIndex] = Pin[pinIndex].Note;
            savePersonaNote(pinIndex, Pin[pinIndex].Note);
          }
          break;
        case 1: ADD(Thresold); break;
        case 2: ADD(ScanTime); break;
        case 3: ADD(MaskTime); break;
        case 4: ADD(Retrigger); break;
        case 5: Pin[eMenuPage-2].Curve=(curve)((Pin[eMenuPage-2].Curve+1)%5); markPinChanged(eMenuPage-2); break;
        case 6: ADD(CurveForm); break;
        case 7: Pin[eMenuPage-2].Xtalk=(Pin[eMenuPage-2].Xtalk+1)%65; markPinChanged(eMenuPage-2); break;
        case 8: Pin[eMenuPage-2].XtalkGroup=(Pin[eMenuPage-2].XtalkGroup+1)%4; markPinChanged(eMenuPage-2); break;
        case 9: // AntiCrosstalk - FORÇA SALVAMENTO DIRETO!
          {
            Pin[pinIndex].AntiCrosstalk = (Pin[pinIndex].AntiCrosstalk + 1) % 128;
            
            // *** SALVA DIRETO NA EEPROM (SEM GetPinSetting) ***
            #if defined(__AVR__)
            int addr = 100 + (pinIndex * 16) + 15;  // Endereço direto
            EEPROM.write(addr, Pin[pinIndex].AntiCrosstalk);  // Valor direto
            
            // DEBUG: Pisca LED para confirmar
            digitalWrite(13, HIGH);
            delay(50);
            digitalWrite(13, LOW);
            #endif
...
void Down()
{
  ...
  { 
    ...
    // *** DENTRO DE SUBMENU: loop reverso entre submenus ***
      ...
      // Se estava no pad A0 (pág 2) com InvSensor e foi para Config → reseta para Note
      // (eMenuPage agora é 1, ou seja saiu do 2)
      if(eMenuPage == 1 && eMenuPin == 15) eMenuPin = 0;
  ...
  else if(eMenuSelect==1) 
  {
    if(eMenuPage==0) Mode=(mode)((int)Mode-1>-1?(int)Mode-1:3);
      ...
      // ✅ Pula índices 4, 6, 8 (ocultos) ao voltar — novos itens 9,10,11,12 visíveis
      if(eMenuAdvanced == 0) {
        eMenuAdvanced = 11;  // Vai para 11 (último visível)
      } else {
        eMenuAdvanced = eMenuAdvanced - 1;
      }
      if(eMenuAdvanced == 8) eMenuAdvanced = 7;   // Pula 8
      else if(eMenuAdvanced == 6) eMenuAdvanced = 5;   // Pula 6
      else if(eMenuAdvanced == 4) eMenuAdvanced = 3;   // Pula 4
    }
    else if(eMenuPage==20) eMenuBackup=eMenuBackup-1>-1?eMenuBackup-1:5;
    else if(eMenuPage==21) eMenuBuzzer=eMenuBuzzer-1>-1?eMenuBuzzer-1:0;
    else if(eMenuPage==22) eMenuVST=eMenuVST-1>-1?eMenuVST-1:0;
    else if(eMenuPage==23) eMenuMidi=eMenuMidi-1>-1?eMenuMidi-1:1;
    else if(eMenuPage==24) eMenuProMicroParam=eMenuProMicroParam-1>-1?eMenuProMicroParam-1:11;
      ...
      byte maxItem = chokeHoldAtivo ? 8 : 4;
      eMenuChokeNote = (eMenuChokeNote - 1 > -1) ? eMenuChokeNote - 1 : maxItem;
    }  // *** NOVO: Página 28 (0-3=notas, 4=filtro, 5-8=hold individual) ***
    // *** MENU XCANCEL (página 34) — navega entre os 16 pares para trás ***
      ...
      if(eMenuConfig == 0) {
        eMenuConfig = 13;  // Se está em 0, volta para 13 (último item)
      } else {
      ...
      eMenuPin=eMenuPin-1>-1?eMenuPin-1:15;
      // InvSensor (15) só aparece no pad A0 (eMenuPage==2)
  ...
  else if(eMenuSelect==2) 
  {
    ...
    else if(eMenuPage==19)
    {
      ...
      else if(eMenuAdvanced==5) { CUSTOM_NOTE_40=(CUSTOM_NOTE_40-1>-1)?CUSTOM_NOTE_40-1:127; markAdvancedChanged(); }
    ...
    {
      if(eMenuVST==0) { 
        currentVSTPreset=(currentVSTPreset-1>-1)?currentVSTPreset-1:3;
        applyVSTPreset(currentVSTPreset);
        if(vstMsgBootAtivo) markVSTChanged();  // so grava na EEPROM se boot msg ativo
        if(vstMsgMenuAtivo) showVSTChangeMessage(currentVSTPreset);
      }
    ...
    else if(eMenuPage==28)
    {
      if(eMenuChokeNote <= 3) {
        ...
        byte idx = eMenuChokeNote - 5;
        chokeHoldMs[idx] = (chokeHoldMs[idx] - 1 + 81) % 81;
        SaveChokeFilterEEPROM();
      }
    }
    ...
    else if(eMenuPage>=2 && eMenuPage<18) 
    {
      ...
      switch(eMenuPin) 
      {
        ...
        case 7: Pin[eMenuPage-2].Xtalk=Pin[eMenuPage-2].Xtalk-1>-1?Pin[eMenuPage-2].Xtalk-1:64; markPinChanged(eMenuPage-2); break;
        case 8: Pin[eMenuPage-2].XtalkGroup=Pin[eMenuPage-2].XtalkGroup-1>-1?Pin[eMenuPage-2].XtalkGroup-1:3; markPinChanged(eMenuPage-2); break;
        case 9: // AntiCrosstalk - FORÇA SALVAMENTO DIRETO!
          {
            Pin[pinIndex].AntiCrosstalk = Pin[pinIndex].AntiCrosstalk - 1 > -1 ? Pin[pinIndex].AntiCrosstalk - 1 : 127;
...
void Draw()
{
  ...
  else if(eMenuPage==22) MenuString(S_VST_MENU,eMenuSelect==0);
  ...
  // *** REMOVIDO: Página 1 não é mais usada ***
    ...
    switch(eMenuPin) {
      ...
      case 9: MenuString(S_ANTICROSSTALK,eMenuSelect==1); break;  // *** NOVO ***
  ...
  // *** CORRIGIDO: Menu Rimshot agora só tem opções 0-8 ***
  else if(eMenuPage==19) {
    switch(eMenuAdvanced) {
      case 0: MenuString(S_ENABLE_37_38,eMenuSelect==1); break;
      case 1: MenuString(S_RIMSHOT,eMenuSelect==1); break;
      case 2: MenuString(S_NOTE_101_102,eMenuSelect==1); break;
      case 3: MenuString(S_NOTE_103_104,eMenuSelect==1); break;
      // case 4: MenuString(S_CUSTOM_NOTE_37,eMenuSelect==1); break;  // OCULTO
      case 5: MenuString(S_CUSTOM_NOTE_40,eMenuSelect==1); break;
      // case 6: MenuString(S_CUSTOM_FORCE_40,eMenuSelect==1); break;  // OCULTO
      case 7: MenuString(S_VEL_THRESH_37_38,eMenuSelect==1); break;
      // case 8: OCULTO
      case 9:  MenuString(S_DETECTION_WINDOW,eMenuSelect==1); break;
      case 10: MenuString(S_MAX_NOTE_AGE,eMenuSelect==1); break;
      case 11: MenuString(S_BLOCK_WINDOW,eMenuSelect==1); break;
      // case 12: MenuString(S_VEL_FILTER,eMenuSelect==1); break;  // OCULTO
      // *** REMOVIDO: casos USB/TX1 ***
    }
  }
  else if(eMenuPage==20) {
    if(eMenuBackup>=0 && eMenuBackup<=2) {
      // Mostra o nome do preset (customizado ou padrão)
      char pname[13];
      loadPresetName(eMenuBackup + 1, pname);
      if(eMenuSelect==1) lcd.print('<');
      lcd.print(pname);
      if(eMenuSelect==1) lcd.print('>');
    } else if(eMenuBackup>=3 && eMenuBackup<=5) {
      // Mostra o nome do preset (customizado ou padrão) para restaurar
      char pname[13];
      loadPresetName(eMenuBackup - 2, pname);
      if(eMenuSelect==1) lcd.print('<');
      lcd.print(pname);
      if(eMenuSelect==1) lcd.print('>');
    }
    ...
    MenuString(S_VST_MENU,eMenuSelect==1);
  }
  // *** Menu Saída MIDI (23) - MANTIDO ***
  else if(eMenuPage==23) {
    if(eMenuMidi==0) {
  ...
  // *** NOVO: Notas Choke (28) ***
    ...
    switch(eMenuChokeNote) {
      ...
      case 4: MenuString(S_CHOKE_FILTER,eMenuSelect==1); break;
      case 5: MenuString(S_CHOKE_FIT_CR1,eMenuSelect==1); break;
      case 6: MenuString(S_CHOKE_FIT_CR2,eMenuSelect==1); break;
      case 7: MenuString(S_CHOKE_FIT_CR3,eMenuSelect==1); break;
      case 8: MenuString(S_CHOKE_FIT_RIDE,eMenuSelect==1); break;
    }
  }
  // *** RESET PADRÃO (29) ***
  else if(eMenuPage==29) {
  ...
  // *** Menu Configurações (25) - OPÇÕES DINÂMICAS ***
    ...
    switch(eMenuConfig) {
      ...
      case 4: MenuString(S_VST_MENU,eMenuSelect==1); break;
      ...
      case 13: MenuString(S_XCANCEL_MENU,eMenuSelect==1); break;
    }
  }
  ...
  if(eMenuPage>=2 && eMenuPage<18) {
    ...
    else if(eMenuPin==4) MenuInt(Pin[pinIndex].Retrigger,eMenuSelect==2);
    else if(eMenuPin==5) {
    ...
    else if(eMenuPin==7) MenuInt(Pin[pinIndex].Xtalk,eMenuSelect==2);
    else if(eMenuPin==8) MenuInt(Pin[pinIndex].XtalkGroup,eMenuSelect==2);
    else if(eMenuPin==9) MenuInt(Pin[pinIndex].AntiCrosstalk,eMenuSelect==2);  // *** NOVO ***
    else if(eMenuPin==10) {
      lcd.setCursor(6,1);
  ...
  // *** CORRIGIDO: Menu Rimshot agora só tem opções 0-8 ***
    ...
    if(eMenuAdvanced==0) MenuInt(ENABLE_NOTE_37_38_TO_40,eMenuSelect==2);
    else if(eMenuAdvanced==1) MenuInt(ENABLE_RIMSHOT_38_TO_40,eMenuSelect==2);
    else if(eMenuAdvanced==2) MenuInt(ENABLE_NOTE_101_TO_102,eMenuSelect==2);
    else if(eMenuAdvanced==3) MenuInt(ENABLE_NOTE_103_TO_104,eMenuSelect==2);
    // else if(eMenuAdvanced==4) MenuInt(CUSTOM_NOTE_37,eMenuSelect==2);  // OCULTO
    else if(eMenuAdvanced==5) MenuInt(CUSTOM_NOTE_40,eMenuSelect==2);
    // else if(eMenuAdvanced==6) MenuInt(CUSTOM_RIMSHOT_FORCE_NOTE,eMenuSelect==2);  // OCULTO
    else if(eMenuAdvanced==7) MenuInt(VELOCITY_THRESHOLD_37_38,eMenuSelect==2);
    // 8 OCULTO
    else if(eMenuAdvanced==9)  MenuInt(DETECTION_WINDOW_MS,eMenuSelect==2);
    else if(eMenuAdvanced==10) MenuInt(MAX_NOTE_AGE_MS,eMenuSelect==2);
    else if(eMenuAdvanced==11) MenuInt(BLOCK_WINDOW_MS,eMenuSelect==2);
    // else if(eMenuAdvanced==12) MenuInt(ENABLE_NOTE_37_38_TO_40,eMenuSelect==2);  // OCULTO
  }
  else if(eMenuPage==20) {
    if(eMenuBackup>=0 && eMenuBackup<=2) {
    ...
    if(eMenuVST==0) {
      lcd.setCursor(7,1);
      if(currentVSTPreset == VST_PERSONA) {
        MenuString(S_PERSONA,eMenuSelect==2);
      } 
      else if(currentVSTPreset == VST_EZDRUMMER) {
        MenuString(S_EZDRUMMER,eMenuSelect==2);
      } 
      else if(currentVSTPreset == VST_SUP2TOMS) {
        MenuString(S_SUP2TOMS,eMenuSelect==2);
      } 
      else {
        MenuString(S_ADDICTIVE,eMenuSelect==2);
      }
    }
  }
  // *** Menu Saída MIDI (23) - MANTIDO ***
  ...
  // *** NOVO: Notas Choke (28) - mostra nota ou status filtro ***
    ...
    if(eMenuChokeNote <= 3) {
      ...
      // Filtro: mostra "Desativ" ou "Ativado"
      if(chokeHoldAtivo == 0) MenuString(S_CHOKE_FILT_OFF, eMenuSelect==2);
      else                    MenuString(PSTR("Ativado"),  eMenuSelect==2);
    } else {
      // Itens 5-8: mostra valor hold (000-080)
      byte idx = eMenuChokeNote - 5;
      MenuInt(chokeHoldMs[idx], eMenuSelect==2);
    }
  }
  // *** Menu Pads Auxiliar (24) ***
    ...
    switch(eMenuProMicroParam) {
      ...
      case 4:
        ...
        lcd.setCursor(12,1); MenuInt(proMicroPads[eMenuProMicro].retrigger, valueSel);
        break;
      case 5:
      ...
      case 9:
        ...
        break;
      case 10:
        MenuString(S_ANTICROSSTALK, paramSel);
        lcd.setCursor(12,1); MenuInt(proMicroPads[eMenuProMicro].antiCrosstalk, valueSel);
        break;
      case 11:

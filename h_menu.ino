...
byte digitalPadNotes[7] = {61, 62,  7,  9, 21, 22, 90};  // Valores padrão Aux1-7 (Addictive)
...
#define S_BACKUP_1 PSTR("Pret 1 Ad")
#define S_BACKUP_2 PSTR("Pret 2 Su") 
#define S_BACKUP_3 PSTR("Preset 3")
#define S_RESTAURAR_1 PSTR("Rest Pret 1 Ad")
#define S_RESTAURAR_2 PSTR("Rest Pret 2 Su")
#define S_RESTAURAR_3 PSTR("Rest Pres 3")
#define S_NOME_PRESET_1 PSTR("Nome Pret 1 Ad")
#define S_NOME_PRESET_2 PSTR("Nome Pret 2 Su")
...
// *** Imprime nome do preset no LCD (conforme número 1-3) ***
void printPresetName(byte num) {
  if(num == 1)      lcd.print(F("Pret 1 Ad"));
  else if(num == 2) lcd.print(F("Pret 2 Su"));
  else              lcd.print(F("Preset 3"));
}
...
void resetToFactoryDefaults() {
  ...
  byte defDigital[7] = {61, 62,  7,  9, 21, 22, 90};
  ...
  // --- Invert Sensor por pad: todos Normal (0) inclusive A0 ---
  ...
  // --- Pads Auxiliar: volta para OCULTO (desativado) ---
  menuPadsAuxiliarAtivo = false;
  modoPadsAuxiliar      = 6;
  maxPadsAuxiliar       = 5;
  SavePadsAuxiliarEEPROM();  // grava endereços 522 e 523

  // --- Salva tudo na EEPROM ---
  forceImmediateSaveToEEPROM();

  // --- Restaura Presets de Fábrica 1 e 2, limpa Preset 3 ---
  WriteFactoryPresets();
...
void Up()
{
  ...
  { 
    ...
    // *** DENTRO DE SUBMENU: loop entre submenus ***
      ...
      // *** Revalida eMenuPin ao trocar de pad ***
  ...
  else if(eMenuSelect==1)
  {
    ...
    // *** MENU TIMING (página 35) — navega entre os 3 parâmetros ***
      ...
      // *** CORREÇÃO: A0 (page==2) — de 15 avança para 16, mas 16 não existe em A0: vai para 0 ***
      if(eMenuPin==16 && eMenuPage!=4 && eMenuPage!=2) eMenuPin=0;
      if(eMenuPin==16 && eMenuPage==2) eMenuPin=0;  // A0: depois do InvSensor(15) volta para Note(0)
      if(eMenuPin==17 && eMenuPage!=4) eMenuPin=0;
      if(eMenuPin==18 && eMenuPage!=4) eMenuPin=0;
    }
  }
...
void Down()
{
  ...
  else if(eMenuSelect==1) 
  {
    ...
    // *** MENU TIMING (página 35) — navega entre os 3 parâmetros para trás ***
      ...
      // Itens 16/17/18: SÓ no pad A2 (page==4) — em qualquer outro pad pula para 14 (Name)
      if(eMenuPin==18 && eMenuPage!=4 && eMenuPage!=2) eMenuPin=14;
      if(eMenuPin==18 && eMenuPage==2)  eMenuPin=15;  // A0: wrap do 0 vai para InvSensor (15)
      if(eMenuPin==17 && eMenuPage!=4) eMenuPin=14;
      // *** CORREÇÃO: A0 (page==2) — de 16 recua para InvSensor (15), não pula para 14 ***
      if(eMenuPin==16 && eMenuPage!=4 && eMenuPage!=2) eMenuPin=14;
      if(eMenuPin==16 && eMenuPage==2) eMenuPin=15;
      // InvSensor (15): só A0 (page==2)

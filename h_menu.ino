...
void Up()
{
  ...
  else if(eMenuSelect==1)
  {
    ...
    // *** MENU TIMING (página 35) — navega entre os 3 parâmetros ***
      ...
      // RishotNote (17): visível só em Comp(0) e Forc(1), não em Des(2)
      if(eMenuPin==17 && eMenuPage==4 && rimSub==2) eMenuPin=0;
      // RishotVel (18): visível só em Comp(0), não em Forc(1) nem Des(2)
      if(eMenuPin==18 && eMenuPage==4 && rimSub!=0) eMenuPin=0;
    }
  }
    ...
    else if(eMenuPage>=2 && eMenuPage<18)
    {
      ...
      switch(eMenuPin)
      {
        ...
        case 16: // Rimshot Comp/Forc/Des — só A2
          rimSub = (rimSub + 1) % 3;  // 0=Comp, 1=Forc, 2=Des
          #if defined(__AVR__)
          ...
          // Des(2) : desativa ambos os rimshotas
          if(rimSub == 0) {
            ENABLE_NOTE_37_38_TO_40 = 1;
            ENABLE_RIMSHOT_38_TO_40 = 0;
          } else if(rimSub == 1) {
          ...
          markAdvancedChanged();
          break;
        case 17: // RishotNote — só A2 (visível em Comp e Forc, não em Des)
          if(rimSub != 2) {
          ...
          break;
        case 18: // RishotVel — velocidade mínima rimshot (só A2, visível só em Comp)
          if(rimSub == 0) {
...
void Down()
{
  ...
  else if(eMenuSelect==1) 
  {
    ...
    // *** MENU TIMING (página 35) — navega entre os 3 parâmetros para trás ***
      ...
      // RishotVel (18): visível só em Comp(0) — ao voltar de 18, pula para 17 se não Comp
      if(eMenuPin==18 && eMenuPage==4 && rimSub!=0) eMenuPin=17;
      // RishotNote (17): visível em Comp(0) e Forc(1) — ao voltar de 17, pula para 16 se Des
    ...
    else if(eMenuPage>=2 && eMenuPage<18) 
    {
      ...
      switch(eMenuPin) 
      {
        ...
        case 15: // InvertSensor — alterna 0/1
          ...
          break;
        case 16: // Rimshot Comp/Forc/Des — só A2
          rimSub = (rimSub == 0) ? 2 : rimSub - 1;  // 0→2→1→0
          #if defined(__AVR__)
          EEPROM.write(EEPROM_RIMSUB_MODE, rimSub);
          #endif
          if(rimSub == 0) {
            ...
            ENABLE_NOTE_37_38_TO_40 = 0;
            ENABLE_RIMSHOT_38_TO_40 = 1;
            CUSTOM_RIMSHOT_FORCE_NOTE = CUSTOM_NOTE_40;
          } else {
            ENABLE_NOTE_37_38_TO_40 = 0;
            ENABLE_RIMSHOT_38_TO_40 = 0;
          }
        ...
        case 17: // RishotNote — só A2 (visível em Comp e Forc, não em Des)
          if(rimSub != 2) {
            ...
            if(rimSub == 1) CUSTOM_RIMSHOT_FORCE_NOTE = CUSTOM_NOTE_40;
            markAdvancedChanged();
          }
          break;
        case 18: // RishotVel — velocidade mínima rimshot (só A2, visível só em Comp)
...
void Draw()
{
  ...
  // *** REMOVIDO: Página 1 não é mais usada ***
    ...
    switch(eMenuPin) {
      ...
      case 17: if(rimSub != 2) MenuString(S_RIMSHOT_NOTE, eMenuSelect==1); break; // *** RishotNote (Comp/Forc) ***
  ...
  if(eMenuPage>=2 && eMenuPage<18) {
    ...
    if(eMenuPin==0) MenuInt(Pin[pinIndex].Note,eMenuSelect==2);
      ...
      else               MenuString(PSTR("Des "),    eMenuSelect==2);
    }
    else if(eMenuPin==17) {  // *** RishotNote (só A2, visível em Comp e Forc) ***
      if(rimSub != 2) MenuInt(CUSTOM_NOTE_40, eMenuSelect==2);
    }
    else if(eMenuPin==18) {  // *** RishotVel — velocidade mínima rimshot (só em Comp) ***

...
void CheckNamesIntegrity()
{
  ...
  for(byte i = 0; i < 16; i++) {
    if(selected_names[i] >= NUM_AVAILABLE_NAMES || selected_names[i] > 34 || selected_names[i] == 255) {
...
void PrintCustomName(byte pin, bool sel)
{
  if(pin < 16) {
    ...
    if(selected_names[pin] >= NUM_AVAILABLE_NAMES || selected_names[pin] > 34 || selected_names[pin] == 255) {
...
void printNameFromIndex(byte nameIdx, bool sel) {
  if(nameIdx >= NUM_AVAILABLE_NAMES || nameIdx > 34) nameIdx = 0;
...
void GetProMicroName(byte padIndex, char* buffer) {
  ...
  if(nameIdx >= NUM_AVAILABLE_NAMES || nameIdx > 34) {
...
void resetToFactoryDefaults() {
  ...
  // --- Restaura notas Persona para valores de fábrica ---
  #if defined(__AVR__)
  {
    // Valores padrão de fábrica da tabela Persona
    const byte defaults[16] = {4,8,38,37,36,47,45,43,51,53,49,57,86,41,48,55};
    for(byte i = 0; i < 16; i++) {
      VST_NOTES_PERSONA[i] = defaults[i];
      EEPROM.write(800 + i, defaults[i]);
    }
  }
  #endif
...
void processEncoder() {
  ...
  if(currentEncoderButtonState) {
    ...
    // Hold longo (3s): ativa atalho PAD
...
void Up()
{
  ...
  { 
    ...
    // *** DENTRO DE SUBMENU: loop entre submenus ***
      ...
      if(eMenuPage == 25) {
        ...
        // *** PULA página 22 (não existe) — de Buzzer(21) vai direto para MIDI(23) ***
        if(prox == 22) prox = 23;
  ...
  else if(eMenuSelect==2)
  {
      ...
      switch(eMenuProMicroParam) {
        ...
        case 11: // Name
          ...
          if(hasCustom) {
            ...
            proMicroPads[eMenuProMicro].nameIndex = (proMicroPads[eMenuProMicro].nameIndex + 1) % NUM_AVAILABLE_NAMES;
    ...
    else if(eMenuPage>=2 && eMenuPage<18)
    {
      ...
      {
        case 0: // Note
          // A0(0)=Controlador, A1(1)=Chimbal, A2(2)=Baixo, A3(3)=Caixa — nota protegida
          if(pinIndex >= 4) {
            ADD(Note);
            if(pinIndex < 16) {
            ...
            lcd.clear();
            lcd.setCursor(0,0); lcd.print("Nota nao pode");
            lcd.setCursor(0,1); lcd.print("ser modificada");
        ...
        case 14:
          if(pinIndex < 16) {
            // A0(0)=Controlador, A1(1)=Chimbal, A2(2)=Baixo, A3(3)=Caixa — nome protegido
            if(pinIndex < 4) {
              lcd.clear();
              lcd.setCursor(0,0); lcd.print("Nome nao pode");
              lcd.setCursor(0,1); lcd.print("ser alterado");
              delay(1500); needsRedraw = true;
            } else {
  ...
  { 
    ...
    // *** DENTRO DE SUBMENU: loop reverso entre submenus ***
      ...
      if(eMenuPage == 25) {
        ...
        byte prox = eMenuPage - 1;
        // *** PULA página 22 (não existe) — de MIDI(23) volta direto para Buzzer(21) ***
        if(prox == 22) prox = 21;
        // *** PROTEÇÃO PAD AUX: página 24 só é acessível via hold longo (botão A médio/longo) ***
      ...
      switch(eMenuProMicroParam) {
        ...
        case 11: // Name
          ...
          if(hasCustom) {
            ...
            proMicroPads[eMenuProMicro].nameIndex = (currentIdx == 0) ? NUM_AVAILABLE_NAMES - 1 : currentIdx - 1;
            saveProMicroPadToEEPROM(eMenuProMicro);
          }
          return;
      }

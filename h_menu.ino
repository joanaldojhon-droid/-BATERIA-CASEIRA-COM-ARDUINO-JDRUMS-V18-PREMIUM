...
void Up()
{
  ...
  else if(eMenuSelect==1)
  {
    ...
    // *** MENU TIMING (página 35) — navega entre os 3 parâmetros ***
      ...
      // Comp(0): 16→17→18  Forc(1): 16→17, pula 18  Des(2): 16 só, pula 17 e 18
      if(eMenuPage==4) {
        if(eMenuPin==17 && rimSub==2) eMenuPin=0;   // Des: pula RishotNote
        if(eMenuPin==18 && rimSub==1) eMenuPin=0;   // Forc: pula RishotVel
        if(eMenuPin==18 && rimSub==2) eMenuPin=0;   // Des: pula RishotVel
      }
    }
  }
    ...
    else if(eMenuPage>=2 && eMenuPage<18)
    {
      ...
      switch(eMenuPin)
      {
        ...
        case 15: // InvertSensor — alterna 0/1
          ...
          break;
        case 16: // Rimshot Comp/Forc/Des — só A2 (0=Comp 1=Forc 2=Des)
          rimSub = (rimSub + 1) % 3;
          #if defined(__AVR__)
          ...
          // Des(2): desativa ambos os rimshoots
          if(rimSub == 0) {
            ENABLE_NOTE_37_38_TO_40 = 1;
            ENABLE_RIMSHOT_38_TO_40 = 0;
          } else if(rimSub == 1) {
...
void Down()
{
  ...
  else if(eMenuSelect==1) 
  {
    ...
    // *** MENU TIMING (página 35) — navega entre os 3 parâmetros para trás ***
      ...
      // Comp(0): 18→17→16  Forc(1): pula 18→17→16  Des(2): pula 18→pula 17→16
      if(eMenuPage==4) {
        if(eMenuPin==18 && rimSub==1) eMenuPin=17;  // Forc: pula RishotVel, vai para RishotNote
        if(eMenuPin==18 && rimSub==2) eMenuPin=16;  // Des: pula RishotVel e RishotNote
        if(eMenuPin==17 && rimSub==2) eMenuPin=16;  // Des: pula RishotNote
      }
      // InvSensor (15): só A0 (page==2)

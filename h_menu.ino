...
bool nameEditMode = false;        // ainda usado pelo ProMicro
char editingName[13];
byte editPosition = 0;
...
bool confirmationMode = false;    // ainda usado pelo ProMicro
...
// *** NOMES PERSONALIZADOS DOS PRESETS DE BACKUP - REMOVIDO ***
...
void Menu()
{
  ...
  if (btnB == LOW && btnA == LOW) {
    ...
    // *** PADS PRINCIPAL (páginas 2-17, parâmetro NAME=14): A+B bloqueado — só lista ***
    else if(eMenuPage >= 2 && eMenuPage < 18 && eMenuPin == 14) {
      return;  // A+B não faz nada no parâmetro Name — apenas navega na lista
    }
...
void Up()
{
  ...
  else if(eMenuSelect==2)
  {
    ...
    else if(eMenuPage>=2 && eMenuPage<18)
    {
      ...
      {
        case 0: // Note
          if(pinIndex <= 3 || currentVSTPreset != VST_PERSONA) { // A0-A3 sempre; outros pads bloqueados fora do Persona
            lcd.clear(); lcd.setCursor(0,0); lcd.print("A nota nao pode"); lcd.setCursor(0,1); lcd.print("ser modificada"); delay(1500); needsRedraw=true;
          } else {
          ADD(Note);
        ...
        case 14:
          if(pinIndex < 16) {
            if(pinIndex <= 3 || currentVSTPreset != VST_PERSONA) { // A0-A3 sempre; outros pads bloqueados fora do Persona
              lcd.clear(); lcd.setCursor(0,0); lcd.print("O nome nao pode"); lcd.setCursor(0,1); lcd.print("ser modificado"); delay(1500); needsRedraw=true;
            } else {
              selected_names[pinIndex] = (selected_names[pinIndex] + 1) % NUM_AVAILABLE_NAMES;
              if(selected_names[pinIndex] >= NUM_AVAILABLE_NAMES) selected_names[pinIndex] = 0;
              markNameChanged();
            }
          }
          break;
        ...
        case 14:
          if(pinIndex < 16) { 
            if(pinIndex <= 3 || currentVSTPreset != VST_PERSONA) { // A0-A3 sempre; outros pads bloqueados fora do Persona
              ...
              if(selected_names[pinIndex] == 0) selected_names[pinIndex] = NUM_AVAILABLE_NAMES - 1;
              else selected_names[pinIndex]--;
              if(selected_names[pinIndex] >= NUM_AVAILABLE_NAMES) selected_names[pinIndex] = 0;
              markNameChanged();
            }
          }

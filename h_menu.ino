...
//========================================================================================//
//=>                         J-DRUMS V9,0 - CONTROLADOR MIDI BATERIA                    <= 
...
void resetToFactoryDefaults() {
  ...
  // --- AntiCrosstalk por pad: zera todos (000) ---
  for(int i = 0; i < 16; i++) {
    Pin[i].AntiCrosstalk = 0;
    SaveEEPROM(i, ANTICROSSTALK);
  }
  ...
  // --- Pads Pro Micro / Auxiliar: zera antiCrosstalk, xCanCost volta para 34 ---
  for(int i = 0; i < NUM_PRO_MICRO_PADS; i++) {
    proMicroPads[i].antiCrosstalk = 0;
    proMicroPads[i].xCanCost      = 34;
    saveProMicroPadToEEPROM(i);
    sendProMicroCommand(0x03, i, PARAM_ANTICROSSTALK, 0);   delay(5);
    sendProMicroCommand(0x03, i, PARAM_XCANCEL,       34);  delay(5);
  }
...
void Menu()
{
  ...
  if (btnB == LOW && btnA == LOW) {
    ...
    // *** PADS AUXILIAR (página 24, parâmetro NAME=12): A+B sem ação (edição removida) ***
    else if(eMenuPage == 24 && eMenuSelect >= 1 && eMenuProMicroParam == 12) {
      // Edição livre de nome removida — usa apenas lista de nomes via encoder/botões
      return;
    }
...
void Up()
{
  ...
  else if(eMenuSelect==1)
  {
    if(eMenuPage==0) Mode=(mode)(((int)Mode+1)%4);
      ...
      eMenuProMicroParam=(eMenuProMicroParam+1)%13;
      if(eMenuProMicroParam==10) eMenuProMicroParam=11;  // AntiCrosstalk oculto — pula para XCancel
    }
    ...
    // *** MENU TIMING (página 35) — navega entre os 3 parâmetros ***
      ...
      if(eMenuPin==8) eMenuPin=10;  // pin 8 removido (XtalkGroup) — pula para Type
      if(eMenuPin==9) eMenuPin=10;  // AntiCrosstalk oculto do menu principal dos pads
  ...
  else if(eMenuSelect==2)
  {
    ...
    else if(eMenuPage==24)
    {
      ...
      switch(eMenuProMicroParam) {
        ...
        case 3: // MaskTime
          ...
          break;
        case 4: // Retrigger — AUT(0)→1→...→16→AUT(0)
          {
        ...
        case 9: // Channel
          ...
          break;
        case 10: // AntiCrosstalk — limitado 0-60
          {
            byte ac = proMicroPads[eMenuProMicro].antiCrosstalk;
            if(ac < 60) ac++; else ac = 0;
            updateProMicroParameter(eMenuProMicro, PARAM_ANTICROSSTALK, ac);
          }
          return;
        case 11: // XCancel — incrementa 0-80 com wrap
          {
            byte xc = proMicroPads[eMenuProMicro].xCanCost;
            if(xc < 80) xc++; else xc = 0;
            updateProMicroParameter(eMenuProMicro, PARAM_XCANCEL, xc);
          }
          return;
        case 12: // Name
          #if defined(__AVR__)
    ...
    else if(eMenuPage>=2 && eMenuPage<18)
    {
      ...
      switch(eMenuPin)
      {
        ...
        case 7: { byte xc = Pin[eMenuPage-2].XCanCost; if(xc < 80) xc++; else xc = 0; Pin[eMenuPage-2].XCanCost = xc; markPinChanged(eMenuPage-2); } break;
        case 8: break;  // XtalkGroup removido — não faz nada
        case 9: // AntiCrosstalk — incrementa 0-60 com wrap
          {
            byte ac = Pin[eMenuPage-2].AntiCrosstalk;
            if(ac < 60) ac++; else ac = 0;
            Pin[eMenuPage-2].AntiCrosstalk = ac;
            SaveEEPROM(eMenuPage-2, ANTICROSSTALK);
            markPinChanged(eMenuPage-2);
          }
          break;
...
void Down()
{
  ...
  else if(eMenuSelect==1) 
  {
    if(eMenuPage==0) Mode=(mode)((int)Mode-1>-1?(int)Mode-1:3);
      ...
      eMenuProMicroParam=eMenuProMicroParam-1>-1?eMenuProMicroParam-1:12;
      if(eMenuProMicroParam==10) eMenuProMicroParam=9;  // AntiCrosstalk oculto — volta para Channel
    }
    ...
    // *** MENU TIMING (página 35) — navega entre os 3 parâmetros para trás ***
      ...
      if(eMenuPin==9) eMenuPin=7;  // AntiCrosstalk oculto do menu principal — volta para XCanCost
      if(eMenuPin==8) eMenuPin=7;  // pin 8 removido (XtalkGroup) — volta para XCanCost
  ...
  else if(eMenuSelect==2) 
  {
    ...
    else if(eMenuPage==24)
    {
      ...
      switch(eMenuProMicroParam) {
        ...
        case 3: // MaskTime
          ...
          paramCode = PARAM_MASKTIME;
          break;
        case 4: // Retrigger — AUT(0)→16→...→1→AUT(0)
          {
            byte &r = proMicroPads[eMenuProMicro].retrigger;
            if(r == 0) r = 16;
            else r--;
        ...
        case 10: // AntiCrosstalk — limitado 0-60
          {
            byte ac = proMicroPads[eMenuProMicro].antiCrosstalk;
            if(ac > 0) ac--; else ac = 60;
            updateProMicroParameter(eMenuProMicro, PARAM_ANTICROSSTALK, ac);
          }
          return;
        case 11: // XCancel — decrementa 0-80 com wrap
          {
            byte xc = proMicroPads[eMenuProMicro].xCanCost;
            if(xc > 0) xc--; else xc = 80;
            updateProMicroParameter(eMenuProMicro, PARAM_XCANCEL, xc);
          }
          return;
        case 12: // Name
    ...
    else if(eMenuPage>=2 && eMenuPage<18) 
    {
      ...
      switch(eMenuPin) 
      {
        ...
        case 7: { byte xc = Pin[eMenuPage-2].XCanCost; if(xc > 0) xc--; else xc = 80; Pin[eMenuPage-2].XCanCost = xc; markPinChanged(eMenuPage-2); } break;
        case 8: break;  // XtalkGroup removido — não faz nada
        case 9: // AntiCrosstalk — decrementa 0-60 com wrap
          {
            byte ac = Pin[eMenuPage-2].AntiCrosstalk;
            if(ac > 0) ac--; else ac = 60;
            Pin[eMenuPage-2].AntiCrosstalk = ac;
...
void Draw()
{
  ...
  // *** REMOVIDO: Página 1 não é mais usada ***
    ...
    switch(eMenuPin) {
      ...
      case 8: break;  // XtalkGroup removido — não exibe
      case 9: MenuString(S_ANTICROSSTALK,eMenuSelect==1); break;  // *** AntiCrosstalk restaurado ***
  ...
  if(eMenuPage>=2 && eMenuPage<18) {
    ...
    else if(eMenuPin==9) MenuInt(Pin[pinIndex].AntiCrosstalk,eMenuSelect==2);  // *** AntiCrosstalk restaurado ***
    else if(eMenuPin==10) {
      lcd.setCursor(6,1);
  ...
  // *** Menu Pads Auxiliar (24) ***
    ...
    switch(eMenuProMicroParam) {
      ...
      case 4:
        ...
        lcd.setCursor(12,1);
        if(proMicroPads[eMenuProMicro].retrigger == 0) MenuString(S_AUT, valueSel);
      ...
      case 10:
        ...
        break;
      case 11:
        MenuString(PSTR("XCancel "), paramSel);

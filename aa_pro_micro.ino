...
// Códigos dos parâmetros (SINCRONIZADO COM PRO MICRO / LEONARDO)
...
#define PARAM_ANTICROSSTALK 0x0A
#define PARAM_XCANCEL    0x0B  // *** XCancel Global — velocity máxima fantasma (0-127) ***

// Estrutura do pad Pro Micro / Leonardo
struct ProMicroPad {
  byte note;
  byte threshold;
  byte scanTime;
  byte maskTime;
  byte retrigger;  // 0=AUT, 1-16=manual (escala igual ao Mega)
  byte curve;
  byte curveForm;
  byte gain;
  ...
  byte xCanCost;       // *** XCancel Global: velocity máxima fantasma (0-127, 0=desligado) ***
  byte nameIndex;  // Índice do nome da lista available_names (0-34)
};
...
void initProMicroPads() {
  ...
  for(int i = 0; i < NUM_PRO_MICRO_PADS; i++) {
    ...
    proMicroPads[i].retrigger = 0;   // 0 = AUT (decay rápido automático)
    ...
    proMicroPads[i].antiCrosstalk = 0;  // Padrão: desligado (0-127)
    proMicroPads[i].xCanCost = 34;      // *** XCancel Global padrão ***
    proMicroPads[i].nameIndex = 15; // Nome padrão = FLENI 2
  }
...
void saveProMicroPadToEEPROM(byte padIndex) {
  ...
  int baseAddr = 4500 + (padIndex * 13);  // 13 bytes por pad
  ...
  EEPROM.write(baseAddr + 11, proMicroPads[padIndex].xCanCost);   // *** XCancel ***
  EEPROM.write(baseAddr + 12, proMicroPads[padIndex].nameIndex);  // nome
  #endif
}
...
void loadProMicroPadsFromEEPROM() {
  ...
  for(int i = 0; i < NUM_PRO_MICRO_PADS; i++) {
    ...
    proMicroPads[i].antiCrosstalk = 0;  // sempre 0 — oculto no menu, ignorar valor da EEPROM
    byte xc = EEPROM.read(baseAddr + 11);
    proMicroPads[i].xCanCost = (xc == 255) ? 34 : xc;  // 255=não init → padrão 34
    byte storedNameIndex = EEPROM.read(baseAddr + 12);
    ...
    // Envia para o Pro Micro / Leonardo
...
void sendAllParametersToProMicro(byte padIndex) {
  ...
  sendProMicroCommand(0x03, padIndex, PARAM_NOTE,          proMicroPads[padIndex].note);          delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_THRESHOLD,     proMicroPads[padIndex].threshold);      delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_SCANTIME,      proMicroPads[padIndex].scanTime);       delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_MASKTIME,      proMicroPads[padIndex].maskTime);       delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_RETRIGGER,     proMicroPads[padIndex].retrigger);      delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_CURVE,         proMicroPads[padIndex].curve);          delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_CURVEFORM,     proMicroPads[padIndex].curveForm);      delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_GAIN,          proMicroPads[padIndex].gain);           delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_TYPE,          proMicroPads[padIndex].type);           delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_CHANNEL,       proMicroPads[padIndex].channel);        delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_ANTICROSSTALK, proMicroPads[padIndex].antiCrosstalk);  delay(5);
  sendProMicroCommand(0x03, padIndex, PARAM_XCANCEL,       proMicroPads[padIndex].xCanCost);       delay(5);
}

// Atualiza parâmetro + salva na EEPROM do Mega + envia para Pro Micro / Leonardo
void updateProMicroParameter(byte padIndex, byte param, byte value) {
  ...
  switch(param) {
    ...
    case PARAM_ANTICROSSTALK: proMicroPads[padIndex].antiCrosstalk = value; break;
    case PARAM_XCANCEL:       proMicroPads[padIndex].xCanCost      = value; break;  // *** XCancel ***
  }
  
  // Envia comando para Pro Micro / Leonardo

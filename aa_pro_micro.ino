...
struct ProMicroPad {
  ...
  byte scanTime;
  byte maskTime;
  byte retrigger;    // Aut=255, 0-16 (decay exponencial)
  byte curve;
  byte curveForm;
  byte gain;
...
void initProMicroPads() {
  ...
  for(int i = 0; i < NUM_PRO_MICRO_PADS; i++) {
    ...
    proMicroPads[i].retrigger = 255;  // Aut — decay exponencial automático
...
void saveProMicroPadToEEPROM(byte padIndex) {
  ...
  int baseAddr = 4500 + (padIndex * 13);  // 13 bytes por pad (xcanCost adicionado)
  ...
  EEPROM.write(baseAddr + 10, proMicroPads[padIndex].xcanCost);   // XCancel Global
  EEPROM.write(baseAddr + 11, proMicroPads[padIndex].nameIndex);  // Nome personalizado
  // baseAddr+12 reservado
  #endif
}
...
void loadProMicroPadsFromEEPROM() {
  #if defined(__AVR__)
  for(int i = 0; i < NUM_PRO_MICRO_PADS; i++) {
    int baseAddr = 4500 + (i * 13);  // 13 bytes por pad
    
    byte storedNote = EEPROM.read(baseAddr + 0);
    if(storedNote == 255) continue;
    ...
    proMicroPads[i].retrigger   = EEPROM.read(baseAddr + 4);  // 255=Aut é válido
    ...
    proMicroPads[i].channel     = EEPROM.read(baseAddr + 9);
    {
      ...
      proMicroPads[i].xcanCost = (storedXcan == 255) ? 34 : storedXcan;
    }
    {
...
void sendAllParametersToProMicro(byte padIndex) {
  ...
  sendProMicroCommand(0x03, padIndex, PARAM_XCANCOST,  proMicroPads[padIndex].xcanCost);  delay(5);
}
...
void updateProMicroParameter(byte padIndex, byte param, byte value) {
  ...
  switch(param) {
    ...
    case PARAM_XCANCOST:  proMicroPads[padIndex].xcanCost  = value; break;  // XCancel Global
  }

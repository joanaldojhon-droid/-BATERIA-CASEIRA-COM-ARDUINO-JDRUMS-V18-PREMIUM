...
byte _valAdvanced(byte param) {
  switch(param) {
    ...
    case 11: return currentVSTPreset;
...
void markVSTChanged() {
  _enqueueEEPROM(_addrAdvanced(11), currentVSTPreset);
}
...
void forceImmediateSaveToEEPROM() {
  ...
  // *** CORRECAO BUG FLAG 537: grava 0xA3 (valor que LoadDigitalNotesEEPROM espera) ***
  ...
  // *** CORRECAO BUG A8/A9: garante que as notas Persona atuais da RAM
  //     sejam gravadas na EEPROM, inclusive após um reset de fábrica.
  //     Sem isso, o próximo boot lia os valores antigos dos endereços 800-815
  //     e A8/A9 apareciam com nota 0 (ou valor desatualizado).
  EEPROM.write(799, 0xAA);  // marca de validação — indica dados presentes
  for(byte i = 0; i < 16; i++) {
    EEPROM.write(800 + i, VST_NOTES_PERSONA[i]);
  }

...
// MAPA DE EEPROM (4096 bytes totais — endereços 0-4095):
//   0    –  999 : dados gerais do sistema (pinos, HH, avançado, nomes, etc.)
//   1000 – 1399 : Preset/Backup 1  (400 bytes)
//   1400 – 1799 : Preset/Backup 2  (400 bytes)
//   1800 – 2199 : Preset/Backup 3  (400 bytes)
//   2200 – 4095 : reservado para expansão futura (~1895 bytes livres)
//
// Tamanho real por preset calculado: ~383 bytes.
// Alocados 400 bytes com ~17 bytes de margem para futuras expansões.
// Nenhum preset ultrapassa o limite de 4095 bytes da EEPROM do Mega 2560.
#define BACKUP_EEPROM_START_1 1000
#define BACKUP_EEPROM_START_2 1400
#define BACKUP_EEPROM_START_3 1800
#define BACKUP_SIZE_1 400
#define BACKUP_SIZE_2 400
#define BACKUP_SIZE_3 400
...
void SaveBackup(byte backupNumber) {
  ...
  EEPROM.write(startAddress + offset++, currentVSTPreset);
  ...
  // *** NOVO: SALVA NOTAS PERSONA (16 bytes) ***
  for(int i = 0; i < 16; i++) {
    EEPROM.write(startAddress + offset++, VST_NOTES_PERSONA[i]);
  }
  ...
  // === DADOS DOS PINOS (13 bytes cada - 16 pinos fixos) ===
  for(int i = 0; i < 16; i++) {
    if(offset + 13 <= (maxSize - 1)) {
...
void RestoreBackup(byte backupNumber) {
  ...
  byte restoredVST = EEPROM.read(startAddress + offset++);
  if(restoredVST <= 3) {  // *** MODIFICADO: aceita 0-3 ***
    currentVSTPreset = restoredVST;
  } else {
  ...
  // *** RESTAURA NOTAS PERSONA (16 bytes) ***
  {
    const byte defaults[16] = {4,8,38,37,36,47,45,43,51,53,49,57,86,41,48,55};
    for(int i = 0; i < 16; i++) {
      byte v = EEPROM.read(startAddress + offset++);
      VST_NOTES_PERSONA[i] = (v <= 127) ? v : defaults[i];
    }
  }
  
  // Salva notas Persona restauradas na EEPROM principal
  for(int i = 0; i < 16; i++) {

...
void SaveBackup(byte backupNumber) {
  ...
  // === NOMES: NÃO SALVA (removido) ===
  // selected_names e custom names não são incluídos no backup
  
  // Placeholder de 1 byte (nenhum nome customizado)
  EEPROM.write(startAddress + offset++, 0); // customCount = 0
  
  EEPROM.write(startAddress + 71, 0); // savedCustomCount = 0
...
void RestoreBackup(byte backupNumber) {
  ...
  // *** NOVO: RESTAURA NOTAS PERSONA (16 bytes) ***
  for(int i = 0; i < 16; i++) {
    VST_NOTES_PERSONA[i] = EEPROM.read(startAddress + offset++);
  ...
  // === NOMES: NÃO RESTAURA (removido) ===
  // selected_names e custom names são preservados como estão
  // Apenas avança o offset pelo byte de customCount (sempre 0 em backups novos)
  offset++; // customCount (ignorado)

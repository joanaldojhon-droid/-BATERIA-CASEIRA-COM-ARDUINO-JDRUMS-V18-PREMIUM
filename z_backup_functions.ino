...
void SaveBackup(byte backupNumber) {
  ...
  EEPROM.write(startAddress + offset++, 0); // reservado (era VST_PRESET)
  ...
  // *** RESERVADO: 16 bytes (era notas Persona) ***
  // Byte 0: estado Pads Auxiliar (0=oculto, 1=ativo)
  // Byte 1: modo Pads Auxiliar (4 ou 6)
  // Bytes 2-15: zeros (reservado futuro)
  EEPROM.write(startAddress + offset++, menuPadsAuxiliarAtivo ? 1 : 0);
  EEPROM.write(startAddress + offset++, modoPadsAuxiliar);
  for(int i = 2; i < 16; i++) {
    EEPROM.write(startAddress + offset++, 0);
  }
...
void RestoreBackup(byte backupNumber) {
  ...
  offset++; // reservado (era VST_PRESET)
  ...
  // Bytes 2-15: zeros (reservado futuro)
  {
    byte auxAtivo = EEPROM.read(startAddress + offset++);
    byte auxModo  = EEPROM.read(startAddress + offset++);
    menuPadsAuxiliarAtivo = (auxAtivo == 1);
    if(auxModo == 4 || auxModo == 6) {
      modoPadsAuxiliar = auxModo;
      maxPadsAuxiliar  = (auxModo == 4) ? 3 : 5;
    } else {
      modoPadsAuxiliar = 6;
      maxPadsAuxiliar  = 5;
    }
    SavePadsAuxiliarEEPROM();  // persiste nos endereços 522 e 523
    offset += 14;  // pula os 14 bytes restantes do bloco reservado
  }
...
// ============================================================
// PRESETS DE FÁBRICA — gravados uma vez na primeira subida
// e restaurados pelo resetToFactoryDefaults().
//
// Preset 1: notas Addictive Drummer padrão (A0-A15)
//           4 8 38 42 36 71 69 67 60 61 77 79 81 65 47 96
//           RinshotNote (CUSTOM_NOTE_40) = 37
//
// Preset 2: notas alternativas (A0-A15)
//           4 5 38 37 47 45 43 51 53 49 57 86 41 48 55  (15 notas — A15 usa padrão 96)
//           RinshotNote (CUSTOM_NOTE_40) = 40
//
// Preset 3: vazio (não existe / "Não Existe")
// ============================================================

// Notas dos presets de fábrica
static const byte PROGMEM FACTORY_NOTES_P1[16] = {
   4,  8, 38, 42, 36, 71, 69, 67, 60, 61, 77, 79, 81, 65, 47, 96
};
static const byte PROGMEM FACTORY_NOTES_P2[16] = {
   4,  5, 38, 37, 47, 45, 43, 51, 53, 49, 57, 86, 41, 48, 55, 96
};

void WriteFactoryPresets() {
  #if defined(__AVR__)

  // --- Guarda estado atual da RAM ---
  byte savedNotes[16];
  for(byte i = 0; i < 16; i++) savedNotes[i] = Pin[i].Note;
  byte savedCustomNote40          = CUSTOM_NOTE_40;
  byte savedCustomRimshotForce    = CUSTOM_RIMSHOT_FORCE_NOTE;
  byte savedDigitalNotes[7];
  for(byte i = 0; i < 7; i++) savedDigitalNotes[i] = digitalPadNotes[i];
  byte savedSelectedNames[16];
  for(byte i = 0; i < 16; i++) savedSelectedNames[i] = selected_names[i];

  // =========================================================
  // PRESET 1
  // =========================================================
  for(byte i = 0; i < 16; i++) {
    Pin[i].Note = pgm_read_byte(&FACTORY_NOTES_P1[i]);
  }
  CUSTOM_NOTE_40            = 37;
  CUSTOM_RIMSHOT_FORCE_NOTE = 37;
  // Notas digitais: Dig1 P45=60, Dig2 P43=61 (demais padrão)
  digitalPadNotes[0] = 60;  // Dig 1 P45
  digitalPadNotes[1] = 61;  // Dig 2 P43
  SaveBackup(1);
  ...
  // =========================================================
  for(byte i = 0; i < 16; i++) {
    Pin[i].Note = pgm_read_byte(&FACTORY_NOTES_P2[i]);
  }
  CUSTOM_NOTE_40            = 40;
  CUSTOM_RIMSHOT_FORCE_NOTE = 40;
  // Notas digitais: Dig1 P45=51, Dig2 P43=53 (demais padrão)
  digitalPadNotes[0] = 51;  // Dig 1 P45
  digitalPadNotes[1] = 53;  // Dig 2 P43
  // Nomes dos pads para o Preset 2
  selected_names[ 7] = 25;  // A7  → SURDO 1
  selected_names[10] = 20;  // A10 → CRASH 1
  selected_names[11] = 21;  // A11 → CRASH 2
  selected_names[12] = 22;  // A12 → CRASH 3
  selected_names[13] = 26;  // A13 → SURDO 2
  selected_names[14] =  5;  // A14 → TOM 1
  selected_names[15] = 31;  // A15 → PAD EFET 1
  SaveBackup(2);

  // =========================================================
  // PRESET 3 — deixa vazio (garante que não existe)
  // =========================================================
  ClearBackup(3);

  // --- Restaura estado original da RAM ---
  for(byte i = 0; i < 16; i++) Pin[i].Note = savedNotes[i];
  CUSTOM_NOTE_40            = savedCustomNote40;
  CUSTOM_RIMSHOT_FORCE_NOTE = savedCustomRimshotForce;
  for(byte i = 0; i < 7; i++) digitalPadNotes[i] = savedDigitalNotes[i];
  for(byte i = 0; i < 16; i++) selected_names[i] = savedSelectedNames[i];

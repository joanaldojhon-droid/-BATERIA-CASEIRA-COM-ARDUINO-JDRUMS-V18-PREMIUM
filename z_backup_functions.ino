...
void SaveBackup(byte backupNumber) {
  ...
  for(int i = 0; i < 16; i++) {
    if(offset + 12 < (startAddress + maxSize - 1)) {
      ...
      EEPROM.write(startAddress + offset++, Pin[i].XCanCost);
    } else {
      break;
    }
  }
...
void RestoreBackup(byte backupNumber) {
  ...
  for(int i = 0; i < 16; i++) {
    ...
    Pin[i].XCanCost = EEPROM.read(startAddress + offset++);
  }
  ...
  // *** SINCRONIZA TRIZONE COM NOTAS RESTAURADAS ***
  // Sem isso o zoneDual_NoteAux/NoteBlock fica com os valores do boot anterior,
  // obrigando desligar/ligar duas vezes para o trizone enxergar as notas corretas.
  for(byte p = 0; p < ZONE_DUAL_PAIRS; p++) {
    zoneDual_NoteAux[p]   = digitalPadNotes[p];
    zoneDual_NoteBlock[p] = Pin[zoneDual_PadIdx[p]].Note;
  }

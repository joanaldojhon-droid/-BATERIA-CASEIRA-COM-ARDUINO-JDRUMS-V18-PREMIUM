...
//                                          p0  p1  p2  p3  p4  p5
const byte   zoneDual_HoldNote[ZONE_DUAL_PAIRS] = {  0, 63,  0,  0, 27, 31 };
...
void updateZoneDual(byte auxIdx, int switchState)
{
  for(byte p = 0; p < ZONE_DUAL_PAIRS; p++) {
    if(digitalPadNotes[auxIdx] == zoneDual_NoteAux[p]) {
      if(switchState == LOW) {
        // Borda de descida: sempre reseta hold se ainda nao disparou
        ...
        if(zoneDual_HoldNote[p] && zoneDual_HoldFired[p]) {
          ...
          // *** FIX CHOKE-RELEASE: mantém HoldFired=true até o latch expirar ***
          // Impede que vibrações mecânicas da soltura disparem a nota de borda
          // no piezo durante o janela do latch (80ms). O watchdog reseta abaixo.
        } else {
          // Toque rápido sem choke: libera flags normalmente

...
// *** DEBOUNCE TEMPORAL CHOKES (Choke1-4) ***
#define CHOKE_DEBOUNCE_MS 20  // Tempo mínimo entre disparos choke (ms) - ajustável
...
void loop()
{
  ...
  if( currentSwitchState == LOW && Choke1_State == HIGH ) { // push
    if(millis() - Choke1_LastTrigger > CHOKE_DEBOUNCE_MS) {
      sendMidiNote(0x09, chokeNotes[1], 127);  // chokeNotes[1] = Crash 2
      Choke1_LastTrigger = millis();
      resetBacklightTimer();
    }
  }
  ...
  if( currentSwitchState == LOW && Choke2_State == HIGH ) { // push
    if(millis() - Choke2_LastTrigger > CHOKE_DEBOUNCE_MS) {
      sendMidiNote(0x09, chokeNotes[2], 127);  // chokeNotes[2] = Crash 3
      Choke2_LastTrigger = millis();
      resetBacklightTimer();
    }
  }
  ...
  if( currentSwitchState == LOW && Choke3_State == HIGH ) { // push
    if(millis() - Choke3_LastTrigger > CHOKE_DEBOUNCE_MS) {
      sendMidiNote(0x09, chokeNotes[3], 127);  // chokeNotes[3] = Ride
      Choke3_LastTrigger = millis();
      resetBacklightTimer();
    }
  }
  ...
  if( currentSwitchState == LOW && ChokeRide_State == HIGH ) { // push
    if(millis() - ChokeRide_LastTrigger > CHOKE_DEBOUNCE_MS) {
      sendMidiNote(0x09, chokeNotes[0], 127);  // chokeNotes[0] = Crash 1
      ChokeRide_LastTrigger = millis();
      resetBacklightTimer();
    }
  }

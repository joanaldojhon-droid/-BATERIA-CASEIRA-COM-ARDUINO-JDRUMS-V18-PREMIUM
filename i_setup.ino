...
void setup()
{
  ...
  byte savedVSTPreset = currentVSTPreset;
  ...
  for(int i = 0; i < 16; i++) {
    ...
    if(valor <= 127) {
      Pin[i].AntiCrosstalk = valor;  // ✅ CARREGA DIRETO NA RAM!
    }
  }
  #endif
  ...
  lcd.begin(16, 2);  
  {
    ...
    // *** CARREGA ESTADO DAS MENSAGENS VST DA EEPROM ***
    LoadVstMsgEEPROM();

    // *** SE BOOT MSG DESATIVADO: força Persona SEM re-aplicar preset ***
    // applyVSTPreset já foi aplicado corretamente no LoadAllEEPROM com notas da EEPROM.
    // Chamar applyVSTPreset aqui novamente quebraria as notas dos pinos A8/A9 no Persona.
    if(!vstMsgBootAtivo) {
      currentVSTPreset = VST_PERSONA;
      // NÃO chama applyVSTPreset — apenas atualiza a variável de controle.
      // As notas dos pinos já estão corretas da EEPROM carregada acima.
    }
    
    // *** MENSAGEM DE BOOT: informa padrão MIDI ativo se NÃO for Persona ***
    if(vstMsgBootAtivo && savedVSTPreset != VST_PERSONA) {
      lcd.clear();
      lcd.setCursor(0, 0);
      switch(savedVSTPreset) {
        case VST_EZDRUMMER: lcd.print("Padrao SupEz1"); break;
        case VST_SUP2TOMS:  lcd.print("Padrao SupEz2"); break;
